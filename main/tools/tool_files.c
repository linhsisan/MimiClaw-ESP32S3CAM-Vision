...
esp_err_t tool_append_file_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    // 驗證路徑安全
    if (!validate_path(path)) {
        snprintf(output, output_size, "Error: path security violation");
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    // 寫入內容並強制換行，確保每一條記憶都是獨立的一行
    fprintf(f, "%s\n", content);
    fclose(f);

    snprintf(output, output_size, "OK: Record saved to %s", path);
    ESP_LOGI(TAG, "append_file: 已將新記憶寫入 %s", path);
