...
static void agent_loop_task(void *arg) {
    ...
    while (1) {
        ...
        while (iteration < MIMI_AGENT_MAX_TOOL_ITER) {
            ...
            for (int i = 0; i < resp.call_count; i++) {
                tool_output[0] = '\0';
                const char *tool_input = resp.calls[i].input;
                char *routed_tool_input = NULL;

                /* Device actions must reply to the chat that triggered this turn.
                 * Never trust or retain a model-generated Telegram destination. */
                if (strcmp(resp.calls[i].name, "take_photo") == 0 &&
                    strcmp(msg_ptr->channel, MIMI_CHAN_TELEGRAM) == 0 &&
                    msg_ptr->chat_id[0] != '\0') {
                    cJSON *input_root = cJSON_Parse(tool_input ? tool_input : "{}");
                    if (!input_root || !cJSON_IsObject(input_root)) {
                        cJSON_Delete(input_root);
                        input_root = cJSON_CreateObject();
                    }
                    cJSON_DeleteItemFromObjectCaseSensitive(input_root, "chat_id");
                    cJSON_AddStringToObject(input_root, "chat_id", msg_ptr->chat_id);
                    routed_tool_input = cJSON_PrintUnformatted(input_root);
                    cJSON_Delete(input_root);
                    if (routed_tool_input) {
                        tool_input = routed_tool_input;
                    }
                }

                tool_registry_execute(resp.calls[i].name, tool_input, tool_output, TOOL_OUTPUT_SIZE);
                free(routed_tool_input);
